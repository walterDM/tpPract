...
<html>
   ...
   <?php 
      ...
      $consulta=mysqli_query($conexion,"SELECT idTipoProducto FROM tiposproductos WHERE idTipoProducto='$categoria'");
      while($r=mysqli_fetch_array($consulta)){
        $idTipoProducto=$r['idTipoProducto'];
      }
      $consulta2 = mysqli_query($conexion, "SELECT p.* from productos as p join productosmarcastp as pp on pp.idTipoProducto=$idTipoProducto and p.idProducto=pp.idProducto AND estado='Activo'");
      $productos_x_pag = 4;
   ...
   <div class="container">
      <div class="row">
          ...
          <?php if (isset($_GET['pagina'])) {
                    ...
                    $consulta3 = mysqli_query($conexion, "SELECT p.* FROM productos as p JOIN productosmarcastp as tp on tp.idTipoProducto=$idTipoProducto AND estado='Activo' and tp.idProducto=p.idProducto limit $iniciar,$productos_x_pag");
                    while ($r = mysqli_fetch_array($consulta3)) { ?>
                                ...
                                <div>
                                    ...
                                    ?>
                                                    <form method="POST" action="modProducto.php">
                                                        <button style="float: left;margin: 5px;border-radius:30px" type="submit" name="idProductos" value="<?php echo $r['idProducto']; ?>" class="btn btn-light"><i class="fas fa-pencil-alt"></i></button>
                                                    </form>
                                          <?php break; 
                                                case "baja producto":
                                          ?>
                        ...
                        <div data-backdrop="static" class="modal" id="info<?php echo $r['idProductos']; ?>">
                                            ...
                                            <div class="col-md-6">
                                               <br>
                                                   <form method="POST" action="ABM.php">
                                                        ...
                                                        <button style="margin-left: 35px;" type="submit" name="idProductos" value="<?php echo $r['idProductos']; ?>" class="btn btn-light">Eliminar</button>
